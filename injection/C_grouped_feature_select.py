...
def extract_grouped_patterns(file_path: str, pattern: str) -> list:
    ...
    if check_secuence(groups):
        return "no patterns to be grouped." #TODO: raise an exception
...
def create_grouped_feature(batch_files: list, func: Callable) -> list:
    ...
    for pdf, (_, pattern) in zip(extractions, batch_files):
        ...
        for extraction in pdf:
            section.append(" ".join(remove_pattern_from_lines(extraction, pattern)).lower().replace(".",""))
    ...
    first_policy_paragraph = [text[:text.find(article)] for text, article in zip(texts, first_articles)]
    ...
    for header in policy_headers:
        ...
        for word in header.split(" "):
            if word.isupper() and word.isalpha():
                new_header.append(word.lower())
        first_policy_headers.append(" ".join(new_header))
    
    # Obtain the policy name by pattern
    matches = [re.search(policy_name_pattern, string=header) for header in first_policy_paragraph]
    first_policy_names = [match[0] if match else "Unnamed Policy" for match in matches]#metadata
    ...
    # Extract the last article content of the middle section (feature articles)
    # recorre la lista de lineas hasta que encuentra una en mayusculas y corta el bucle
        ...
        for policy in pdf:
            ...
            for line in policy:
                if not line.isupper():
                    ...
                    content_section.append(" ".join(content))
                    break
        last_articles_content.append(content_section)    
    ...
    # FEATURE FILES
    """
    page_content: header + article -> removed pattern and in lowercase
    metadata: policy
    
    articles: son el page_content sin el header
    first_policy_headers: son los header del primer policy de cada file
    middle_policies_header: son los header de las policies complementarias del file
    
    
    first_policy_names: es la metadata del primer policy de cada file
    middle_policies_names: es la metadata de las policies complementarias del file
    
    - object structure:
    
    articles: [ ["policy1_pagecontent", "policy2_pagecontent"], [], ... ]
    ...
    for i in range(len(first_policy_headers)):
        ...
        file_content_hip = [" ".join(element) for element in list(zip(headers, articles[i]))]
        policy_names = [first_policy_names[i]] + middle_policies_names[i]
        
        for j in range(len(policy_names)):
            feature_files.append(
                Document(
                    page_content=file_content_hip[j],
                    metadata={
...
if __name__ == "__main__":
    ...
    # pdf_files = ["POL320190074.pdf"]
    patterns = ["^ART.CULO\s(?:N.\s)?(\d+).:?"]
    
    # Obtain the features data
    file_batch = create_file_batch(pdf_files, extract_grouped_patterns, *patterns)
    
    # First Feature: Extracting the article titles and policy header
    feature_files = create_grouped_feature(file_batch, extract_grouped_patterns)
    
    # Second Feature: Extracting the article headers from policies
    # feature_articles = create_feature_article(file_batch, extract_grouped_patterns)
