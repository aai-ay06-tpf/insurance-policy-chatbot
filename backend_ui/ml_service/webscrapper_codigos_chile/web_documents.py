...
def obtain_scrapped_documents(web_id: str, articles: list) -> list:
    ...
    for article_number in articles:
        ...
        # Load the data
        loader = WebBaseLoader(DOCUMENTS_INFO[web_id].format(
            article_number=article_number))
        data = loader.load()

        # Preprocessing results
        cleaned_lines = [line.replace('\xa0', ' ').strip(
        ) for line in data[0].page_content.splitlines() if line.strip()]
        ...
        if cleaned_lines:
            ...
            web_feature.append(
                Document(
                    ...
                    metadata={
                        "url": DOCUMENTS_INFO[web_id].format(article_number=article_number),
                        "article_number": article_number,
                    }
                )
            )
            ...
            raise ValueError(
                f"Content not found for article {article_number}.")
